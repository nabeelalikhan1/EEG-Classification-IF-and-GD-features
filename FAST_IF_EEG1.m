...
function [fidexmult,ext_sig,res_signal] = FAST_IF_EEG1(Sig,win_length, num, delta,L,thr,Thr)
...
for iii=1:num
    ...
    if sum(abs(s2(length(Sig)/2-LL-1:length(Sig)/2+LL-1).^2))<e_max*thr
        ...
        Sig1=zeros(1,length(s2));
        Sig1(length(Sig)/2-LL:length(Sig)/2+LL)=s2(length(Sig)/2-1*LL:length(Sig)/2+LL);
        Sig1=ifft(ifftshift(Sig1)).*conj(s_dechirp);
